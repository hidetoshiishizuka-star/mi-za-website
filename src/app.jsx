...
function Member({ go }) {
  const MEMBERS = [
    { role: "CEO", img: "photo-ceo.jpg", alt: "在宅医療・訪問診療クリニック専門 経営コンサルタント・事務長支援 石塚美絵（波村美絵）代表取締役 MI-ZA", name: "代表取締役　石塚（波村）美絵", bio: "九州大学大学院法学府を修了後、琉球大学で法学を教えていました。学生に法を説きながら、法律は弱い立場の人を守る力を持つ一方で、その人を直接支えるには遠すぎる——そう感じるようになりました。もっと身近で人を支えたい。その思いから、病院のMSW（医療ソーシャルワーカー）へ転じます。患者さんと向き合うなかで気づいたのは、病院では見えない暮らしがあるということでした。その人の家に上がり、家族を知り、人生ごと診る。在宅医療の考えに深く共鳴し、この世界に入りました。身近な人が在宅医療を知らないまま苦しんだ経験も、この道を選ぶ後押しになりました。訪問診療クリニックの事務長として約10年、運営に携わってきました。仕組み化の支援により、残業に苦しんでいたスタッフの残業を半年でゼロに。スタッフが前に進めなくなったときの合意形成を、組織開発の一助として支えています。在宅医療を必要とする人に、必要な医療が届く——その仕組みを支えることが、私の役割だと考えています。", skill: "得意領域：組織の意思決定の流れをつくる（組織プロセスファシリテーション）、診療報酬改定対応、医療広告・HP運用、人事労務整備、医師採用・定着支援、事務長・管理職育成", career: [
